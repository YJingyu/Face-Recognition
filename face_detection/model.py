...
class model:
	def __init__(self, path = None):
		...
		self.label_boolean = tf.reshape(tf.cast(self.labels / 255, dtype = tf.bool), shape = [-1])
		self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.net.output, labels = tf.cast(self.labels / 255, dtype = tf.float32)))
	...
	def segment(self, batch_inputs):
		output = self.session.run(255 * tf.cast((self.net.segmented_output >= THRESHOLD), dtype = tf.int32), feed_dict = {self.net.input_layer: batch_inputs})
		return output
